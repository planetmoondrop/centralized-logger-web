---
title: Introduction
description: Get started with Moondrop Centralized Logger for NestJS observability.
---
...
Under the hood, [`centralized-logger`](/docs/introduction#packages) is built on a robust HTTP server powered by [`nest`](https://nestjs.com/), ensuring scalable, reliable, and production-ready request handling.

This library documents the two npm packages that make up the stack.

## Packages

- [`@planetmoondrop/centralized-logger`](https://www.npmjs.com/package/@planetmoondrop/centralized-logger) — NestJS module that ships structured logs to Grafana Loki, exposes Prometheus `/metrics`, and integrates with Tempo via OpenTelemetry.
- [`@planetmoondrop/logger-client`](https://www.npmjs.com/package/@planetmoondrop/logger-client) — Axios helper for browsers that keeps `x-loki-trace-id` flowing across frontend to backend calls.
